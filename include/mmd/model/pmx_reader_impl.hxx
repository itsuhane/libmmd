...
inline Model* PmxReader::Read(FileReader &file) const {
    ...
    try {
        ...
        size_t extra_UV_number = file.Read<std::uint8_t>();
        model->SetExtraUVNumber(extra_UV_number);

        size_t vertex_index_size = file.Read<std::uint8_t>();
        size_t texture_index_size = file.Read<std::uint8_t>();
        size_t material_index_size = file.Read<std::uint8_t>();
        size_t bone_index_size = file.Read<std::uint8_t>();
        size_t morph_index_size = file.Read<std::uint8_t>();
        size_t rigid_body_index_size = file.Read<std::uint8_t>();

        model->SetName(file.ReadString(utf8_encoding));
        model->SetNameEn(file.ReadString(utf8_encoding));
        model->SetDescription(file.ReadString(utf8_encoding));
        model->SetDescriptionEn(file.ReadString(utf8_encoding));
        ...
        for(size_t i=0;i<vertex_num;++i) {
            ...
            case Model::SkinningOperator::SKINNING_BDEF2:
                op.GetBDEF2().SetBoneID(0, file.ReadIndex(bone_index_size));
                op.GetBDEF2().SetBoneID(1, file.ReadIndex(bone_index_size));
                op.GetBDEF2().SetBoneWeight(file.Read<float>());
                break;
            case Model::SkinningOperator::SKINNING_BDEF4:
                for(size_t j=0;j<4;++j) {
                    op.GetBDEF4().SetBoneID(j, file.ReadIndex(bone_index_size));
                }
                for(size_t j=0;j<4;++j) {
                    op.GetBDEF4().SetBoneWeight(j, file.Read<float>());
                }
                break;
            case Model::SkinningOperator::SKINNING_SDEF:
                op.GetSDEF().SetBoneID(0, file.ReadIndex(bone_index_size));
                op.GetSDEF().SetBoneID(1, file.ReadIndex(bone_index_size));
                op.GetSDEF().SetBoneWeight(file.Read<float>());
                op.GetSDEF().SetC(file.Read<Vector3f>());
                op.GetSDEF().SetR0(file.Read<Vector3f>());
        ...
        for(size_t i=0;i<part_num;++i) {
            ...
            if(use_global_toon) {
                size_t global_toon_index = file.Read<std::uint8_t>();
            ...
            size_t part_triangle_num = (size_t)file.Read<std::int32_t>()/3;
        ...
        size_t bone_num = (size_t)file.Read<std::int32_t>();
        for(size_t i=0;i<bone_num;++i) {
            Model::Bone &bone = model->NewBone();
            bone.SetName(file.ReadString(utf8_encoding));
            bone.SetNameEn(file.ReadString(utf8_encoding));
            bone.SetPosition(file.Read<Vector3f>());
            size_t parent_index = file.ReadIndex(bone_index_size);
            if(parent_index<bone_num) {
                bone.SetParentIndex(parent_index);
            } else {
                bone.SetParentIndex((size_t)(-1));
            }
            bone.SetTransformLevel((size_t)file.Read<std::int32_t>());
            std::uint16_t flag = file.Read<std::uint16_t>();
            ...
            if(bone.IsChildUseID()) {
                ...
                bone.SetChildOffset(file.Read<Vector3f>());
            }
            if(bone.IsAppendRotate()||bone.IsAppendTranslate()) {
                bone.SetAppendIndex(file.ReadIndex(bone_index_size));
                bone.SetAppendRatio(file.Read<float>());
            }
            if(bone.IsRotAxisFixed()) {
                bone.SetRotAxis(file.Read<Vector3f>());
            }
            if(bone.IsUseLocalAxis()) {
                Vector3f local_x = file.Read<Vector3f>();
                Vector3f local_z = file.Read<Vector3f>();
                Vector3f local_y;
            ...
            if(bone.IsReceiveTransform()) {
                bone.SetExportKey((size_t)file.Read<std::int32_t>());
            }
            if(bone.IsHasIK()) {
                bone.SetIKTargetIndex(file.ReadIndex(bone_index_size));
                bone.SetCCDIterateLimit((size_t)file.Read<std::int32_t>());
                bone.SetCCDAngleLimit(file.Read<float>());
                size_t ik_link_num = (size_t)file.Read<std::int32_t>();
                for(size_t j=0;j<ik_link_num;++j) {
                    Model::Bone::IKLink &link = bone.NewIKLink();
                    link.SetLinkIndex(file.ReadIndex(bone_index_size));
                    link.SetHasLimit(file.Read<std::int8_t>()!=0);
                    if(link.IsHasLimit()) {
                        link.SetLoLimit(file.Read<Vector3f>());
                        link.SetHiLimit(file.Read<Vector3f>());
                    }
                }
            }
        }

        size_t morph_num = (size_t)file.Read<std::int32_t>();
        for(size_t i=0;i<morph_num;++i) {
            ...
            case Model::Morph::MORPH_TYPE_MATERIAL:
                for(size_t j=0;j<morph_data_num;++j) {
                    Model::Morph::MorphData &morph_data = morph.NewMorphData();
                    size_t mm_index = file.ReadIndex(material_index_size);
        ...
        size_t rigid_body_num = (size_t)file.Read<std::int32_t>();
